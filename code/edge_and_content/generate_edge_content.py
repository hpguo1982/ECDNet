...
def split_map(datapath, datapath1):
    ...
    for name in os.listdir(datapath):
        ...
        if not os.path.exists(datapath1+'/content/'):
            os.makedirs(datapath1+'/content/')
        cv2.imwrite(datapath1+'/content/'+name, body)

        if not os.path.exists(datapath1+'/edge/'):
            os.makedirs(datapath1+'/edge')
        cv2.imwrite(datapath1+'/edge/'+name, mask-body)
