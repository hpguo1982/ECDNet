...
class Dataset(data.Dataset):
    def __init__(
        ...
        if mode == 'train':
            ...
            content_paths = os.path.join(folder, "content/")
            self.content_paths = [p for ext in exts for p in Path(f'{content_paths}').glob(f'**/*.{ext}')]
            edge_paths = os.path.join(folder, "edge/")
            self.edge_paths = [p for ext in exts for p in Path(f'{edge_paths}').glob(f'**/*.{ext}')]
            ...
            self.transform = A.Compose([
                ...
                # ToTensorV2()
            ], additional_targets={'content': 'mask', 'edge': 'mask'})
        elif mode == 'test':
    ...
    def Normalize(self, image, mask=None, content=None, edge=None):
        # image = (image - self.mean) / self.std
        if mask is None:
            return image
        if content is None:
            return image, mask / 255
        return image, mask / 255, content/255, edge/255
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            content_paths = self.content_paths[index]
            content = Image.open(content_paths)
            content = convert_image_to_fn(self.convert_image_to, content)
            # content = convert_image_to_fn(self.convert_image_to, content).astype('int32')

            edge_paths = self.edge_paths[index]
            edge = Image.open(edge_paths)
            edge = convert_image_to_fn(self.convert_image_to, edge)
            # edge = convert_image_to_fn(self.convert_image_to, edge).astype('int32')

            # 提取文件名
            img_name = os.path.basename(img_paths)
            mask_name = os.path.basename(mask_paths)
            content_name = os.path.basename(content_paths)
            edge_name = os.path.basename(edge_paths)

            # 检查文件名是否相同
            assert img_name == mask_name == content_name == edge_name, \
                f"Files do not match: {img_name}, {mask_name}, {content_name}, {edge_name}"
            # content = np.where(np.array(content) > 1, 1, np.array(content))
            # edge = np.where(np.array(edge) >1, 1, np.array(edge))
            # transform = self.transform(image=np.array(img), mask=np.array(mask), content=content,
            #                            edge=edge)

            transform = self.transform(image=np.array(img), mask=np.array(mask), content=np.array(content),
                                       edge=np.array(edge))
            image_data = transform['image']
            mask_data = transform['mask']
            content_data = transform['content']
            edge_data = transform['edge']

            # cv2.imwrite('./image_data.jpg', image_data)
            # cv2.imwrite('./mask_data.jpg', mask_data)
            # cv2.imwrite('./content_data.jpg', content_data*255)
            # cv2.imwrite('./edge_data.jpg', edge_data*255)
            mask_data = np.where(mask_data > 64, 255, 0)
            image_data, mask_data, content_data, edge_data = self.Normalize(image_data, mask_data, content_data,
                                                                           edge_data)

            image_data = torch.from_numpy(image_data).float().unsqueeze(dim=0)
            mask_data = torch.from_numpy(mask_data).float().unsqueeze(dim=0)
            content_data = torch.from_numpy(content_data).float().unsqueeze(dim=0)
            edge_data = torch.from_numpy(edge_data).float().unsqueeze(dim=0)
            ...
            content_data2 = np.array(content_data)
            edge_data2 = np.array(edge_data)

            # return image_data, mask_data, content_data, edge_data, class_lable
            return image_data, mask_data, content_data, edge_data
...
if __name__ == '__main__':
    ...
    train_dataset = Dataset(" ", 256, 'train', convert_image_to='L')
    test_dataset = Dataset(" ", 256, 'test', convert_image_to='L')
    ...
    for i, (data) in enumerate(train_qus):
        ...
        image_data, mask_data, content_data, edge_data = data
        print(i)
